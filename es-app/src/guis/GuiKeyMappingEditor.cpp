...
//		mTargetText->setText(mTarget.toTargetString() + _U("      \uF05A ") + target.description);
	...
	mDescription = std::make_shared<TextComponent>(mWindow, mTarget.description.empty() ? "" : std::string(_U("\uF05A  ")) + es_pgettext("keys_files", mTarget.description.c_str()), theme->Text.font, theme->Text.color);
